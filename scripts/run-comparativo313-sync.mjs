#!/usr/bin/env node
/**
 * Dispara manualmente o sync do agente comparativo313.
 *
 * Uso:
 *   node scripts/run-comparativo313-sync.mjs
 *   node scripts/run-comparativo313-sync.mjs --url http://10.0.0.20:3000
 *
 * Lê SYNC_TOKEN de backend/.env automaticamente.
 *
 * O backend precisa estar rodando (`cd backend && npm run dev`).
 * Este script é o equivalente externo do COMPARATIVO313_SYNC_CRON:
 * útil para disparar via agendador do Windows / cron do SO no lugar
 * do cron interno do node-cron.
 */
...
const url = `${base.replace(/\/$/, '')}/agente/comparativo313/run`;
