...
const linhas = data || [];
if (linhas.length === 0) {
  console.log('Nenhuma lacuna a preencher — todos os dias do intervalo já têm foto real');
  console.log('(ou não há snapshot real recente o suficiente para servir de base).');
  process.exit(0);
}

let total = 0;
for (const r of linhas) {
  const d = r.data_derivada;
  const s = r.base_snapshot;
  const n = Number(r.linhas_derivadas || 0);
  total += n;
  console.log(
    `  ${fmtBR(d)} (${semana(d)})  ←  base ${fmtBR(s)} (${semana(s)})   ` +
    `${n.toLocaleString('pt-BR')} linha(s) derivada(s)`
  );
}

console.log(`\n✓ ${linhas.length} dia(s) reconstruído(s), ${total.toLocaleString('pt-BR')} linhas em ${((Date.now() - t0) / 1000).toFixed(1)}s`);
