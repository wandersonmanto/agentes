-- =====================================================================
-- fn_vendas_resumo: acrescenta lucro_liquido e margem_liquida.
--   lucro_liquido  = venda − custo + imp_saida + pis_cofins_lucro
--                    (imp_saida e pis vêm NEGATIVOS na origem, então somá-los
--                     equivale a descontar o imposto)
--   margem_liquida = lucro_liquido / venda * 100
-- Como o tipo de retorno muda, é preciso DROP + CREATE.
-- =====================================================================
drop function if exists public.fn_vendas_resumo(text, date, date, jsonb);

create function public.fn_vendas_resumo(
  p_dim     text,
...
begin
  ...
  v_sql := format($f$
    select
      ...
      -- líquido: desconta os impostos (imp_saida e pis vêm negativos)
      (coalesce(sum(venda_valor),0) - coalesce(sum(custo_liquido),0)
        + coalesce(sum(imp_saida),0) + coalesce(sum(pis_cofins_lucro),0)) as lucro_liquido,
      case when coalesce(sum(venda_valor),0) <> 0
           then round((coalesce(sum(venda_valor),0) - coalesce(sum(custo_liquido),0)
                       + coalesce(sum(imp_saida),0) + coalesce(sum(pis_cofins_lucro),0))
                      / sum(venda_valor) * 100, 2)
